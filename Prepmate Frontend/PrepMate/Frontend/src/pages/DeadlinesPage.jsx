...
import { CalendarDays, AlertCircle, Clock, CheckCircle2, Archive, Plus, Trash2, X, RefreshCw } from 'lucide-react';
import { useAuth } from '../context/authContext';
import { getDeadlines, createDeadline, updateDeadline, deleteDeadline as deleteDeadlineApi } from '../services/api';

const DeadlinesPage = () => {
  const { backendUserId } = useAuth();
  const [deadlines, setDeadlines] = useState([
    { id: 1, title: 'GATE 2027 Exam', date: '2027-02-05', type: 'Exam', priority: 'High', completed: false },
    { id: 2, title: 'Scholarship Form Submission', date: '2026-04-15', type: 'Form', priority: 'Medium', completed: false },
    { id: 3, title: 'Semester V Results', date: '2026-03-20', type: 'Result', priority: 'Low', completed: false },
    { id: 4, title: 'Internship Application', date: '2026-01-10', type: 'Application', priority: 'High', completed: true },
  ]);
  ...
  useEffect(() => {
    ...
    getDeadlines(backendUserId).then(setDeadlines).catch(console.error);
  }, [backendUserId]);
  ...
  const toggleComplete = (id) => {
    ...
    if (!deadline) return;
    const current = !!deadline.completed;
    updateDeadline(id, { completed: !current }).then(updated => setDeadlines(prev => prev.map(d => d.id === id ? updated : d))).catch(console.error);
  };

  const deleteDeadline = (id) => {
    deleteDeadlineApi(id).then(() => setDeadlines(prev => prev.filter(d => d.id !== id))).catch(console.error);
  };

  const addDeadline = (e) => {
    e.preventDefault();
    if (!newDeadline.title || !newDeadline.date) return;
    const payload = { ...newDeadline, completed: false };
    createDeadline(backendUserId, payload).then(saved => setDeadlines(prev => [...prev, saved])).catch(console.error);
                      ...
                      <div className="flex justify-between items-start line-through decoration-slate-400 dark:decoration-slate-600">
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            ...
                            className="text-slate-400 dark:text-slate-600 hover:text-blue-500 dark:hover:text-blue-400 p-1"
                            title="Mark as active"
                          >
                            <RefreshCw size={14}/>
                          </button>
                          <button onClick={() => deleteDeadline(deadline.id)} className="text-slate-400 dark:text-slate-600 hover:text-red-500 dark:hover:text-red-400 p-1"><X size={14}/></button>
                        </div>
                      </div>
