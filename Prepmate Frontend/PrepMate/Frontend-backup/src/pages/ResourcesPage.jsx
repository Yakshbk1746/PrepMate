...
import { FolderOpen, FileText, Upload, Plus, Download, Trash2, Search, Filter, MoreVertical, Heart, FileIcon, ImageIcon, VideoIcon, X } from 'lucide-react';
import { useAuth } from '../context/authContext';
import { getResources, createResource, toggleResourceFavorite, deleteResource as deleteResourceApi } from '../services/api';
...
const ResourcesPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchResources = async () => {
      try {
        const res = await getResources(dbUser.id);
  ...
  const toggleFavorite = async (id) => {
    try {
      await toggleResourceFavorite(id);
      setFiles(files.map(f => f.id === id ? { ...f, favorite: !f.favorite } : f));
    } catch (error) {
  ...
  const confirmUpload = async (e) => {
    e.preventDefault();
    if (!stagedFile || !uploadFormData.name.trim() || !dbUser?.id) return;

    const ext = uploadFormData.name.split('.').pop().toLowerCase();
    let type = 'doc';
    if (ext === 'pdf') type = 'pdf';
    else if (['png', 'jpg', 'jpeg'].includes(ext)) type = 'image';
    else if (['mp4', 'mkv', 'webm'].includes(ext)) type = 'video';

    const newFile = {
      name: uploadFormData.name,
      category: uploadFormData.category,
      type: type,
      size: uploadFormData.sizeStr,
      date: new Date().toISOString().split('T')[0],
      favorite: false
      // Important: actual file content uploading is omitted for simplicity/mock,
      // you would normally upload to S3/Firebase Storage and save the URL.
    };
    
    try {
      const res = await createResource(dbUser.id, newFile);
      ...
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <button className="flex items-center gap-2 px-4 py-2.5 bg-slate-100 dark:bg-[#0a0f1e]/60 border border-slate-200 dark:border-white/[0.06] hover:bg-slate-200 dark:hover:bg-white/[0.1] text-slate-800 dark:text-white text-sm font-bold uppercase tracking-wider rounded-xl transition-colors shadow-sm">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <div className="relative inline-block group/menu">
                      ...
                      <div className="absolute right-0 top-full mt-1 w-32 bg-white dark:bg-[#111827] border border-slate-200 dark:border-white/10 rounded-xl shadow-xl opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all z-10 flex flex-col p-1">
                        <button className="text-left px-3 py-2 text-xs font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 rounded-lg flex items-center gap-2">
